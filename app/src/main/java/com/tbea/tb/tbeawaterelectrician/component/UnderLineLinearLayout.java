...
public class UnderLineLinearLayout extends LinearLayout {
    ...
    public UnderLineLinearLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        lineMarginSide = attr.getDimensionPixelOffset(R.styleable.UnderLineLinearLayout_line_margin_side, 25);
        ...
        lineColor = attr.getColor(R.styleable.UnderLineLinearLayout_line_color, 0xffBFBFBF);
        pointSize = attr.getDimensionPixelSize(R.styleable.UnderLineLinearLayout_point_size, 8);
        pointColor = attr.getDimensionPixelOffset(R.styleable.UnderLineLinearLayout_point_color, 0xffBFBFBF);
    ...
    //=============================================================Vertical Draw
    private void drawFirstChildViewVertical(Canvas canvas) {//最后一个圆点
...
//        }
    ...
    private void drawLastChildViewVertical(Canvas canvas) {//第一个图
//        if (getChildAt(getChildCount() - 1) != null) {
//            int top = getChildAt(getChildCount() - 1).getTop();
//            //记录值最后的位置
//            lastX = (sideRelative>=rootMiddle?(sideRelative-lineMarginSide):(sideRelative+lineMarginSide)) - (mIcon
...
//        }
        if (getChildAt(0) != null) {
            int top = getChildAt(0).getTop();
            //记录值
            firstX = (sideRelative>=rootMiddle?(sideRelative-lineMarginSide):(sideRelative+lineMarginSide))- (mIcon.getWidth()>> 1);
            firstY = top + getChildAt(0).getPaddingTop() + lineDynamicDimen;
            //画一个图
            canvas.drawBitmap(mIcon,firstX-(mIcon.getWidth()/2),firstY-(mIcon.getWidth()/2),null);
//            canvas.drawCircle(firstX, firstY, pointSize, pointPaint);
        }
    }
