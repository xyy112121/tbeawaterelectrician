...
public class CustomScrollView extends View {
    ...
    private List<Condition> mDataList;
    /**
    ...
    private void performSelect()
    {
        if (mSelectListener != null)
            mSelectListener.onSelect(mDataList.get(mCurrentSelected).getId(),mDataList.get(mCurrentSelected).getName());
    }

    public void setData(List<Condition> datas)
    {
        mDataList = datas;
    ...
    {
        Condition head = mDataList.get(0);
        mDataList.remove(0);
        mDataList.add(head);
    }
    ...
    {
        Condition tail = mDataList.get(mDataList.size() - 1);
    ...
    {
//        TextView linearLayout = (TextView) mActivity.getLayoutInflater().inflate(R.layout.pop_window_scrollview, null,
//                false);
//        // 先绘制选中的text再往上往下绘制其余的text
        float scale = parabola(mViewHeight / 4.0f, mMoveLen);
        float size = (mMaxTextSize - mMinTextSize) * scale;
        mPaint.setTextSize(size);
        mPaint.setAlpha((int) ((mMaxTextAlpha - mMinTextAlpha) * scale + mMinTextAlpha));
//        // text居中绘制，注意baseline的计算才能达到居中，y值是text中心坐标
        float x = (float) (mViewWidth / 2.0);
        float y = (float) (mViewHeight / 5.0 + mMoveLen);
...
//          addView(linearLayout);
    ...
    private void drawOtherText(Canvas canvas, int position, int type)
    {
        ...
        mPaint.setTextSize(size);
        mPaint.setAlpha((int) ((mMaxTextAlpha - mMinTextAlpha) * scale + mMinTextAlpha));
        float y = (float) (mViewHeight / 5.0 + type * d);
        Paint.FontMetricsInt fmi = mPaint.getFontMetricsInt();
        float baseline = (float) (y - (fmi.bottom / 2.0 + fmi.top / 2.0));
    ...
    {
        void onSelect(String text,String name);
    }
}
